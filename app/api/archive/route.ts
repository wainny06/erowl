...
import { compareRecords, comparisonCsv, type ArchiveSource } from '@/lib/archive-model';
import { json, sameOrigin } from '@/lib/http';
import { validDate } from '@/lib/daily-model';
export const dynamic='force-dynamic';
export const runtime='nodejs';
export async function GET(req:Request){
 const u=new URL(req.url),source=u.searchParams.get('source')||'legacy';
 if(!['legacy','daily'].includes(source))return json({error:'장부 종류를 확인해 주세요.'},400);
 try{
  const kind=source as ArchiveSource,index=await archiveIndex(kind),date=u.searchParams.get('date');
  ...
  const [record,previous]=await Promise.all([rawRecord(kind,date),baseline?rawRecord(kind,baseline):Promise.resolve(null)]);
  if(!record||baseline&&!previous)return json({error:'장부를 읽지 못했습니다. 다시 불러와 주세요.'},503);
  const rows=compareRecords(record,previous,kind);
  if(u.searchParams.get('format')==='csv')return new Response(comparisonCsv(date,baseline,rows),{headers:{'Content-Type':'text/csv; charset=utf-8','Content-Disposition':`attachment; filename="kmed-${source}-${date}-changes.csv"`,'Cache-Control':'no-store'}});
...
export async function DELETE(req:Request){
 ...
 try{const raw=await req.text();if(raw.length>1000)return json({error:'입력이 너무 깁니다.'},400);const b=JSON.parse(raw);
  if(!b||!['legacy','daily'].includes(b.source)||typeof b.date!=='string'||!validDate(b.date)||b.confirm!==b.date)return json({error:'삭제할 날짜를 정확히 입력해 주세요.'},400);
  if(!await deleteRecord(b.source,b.date))return json({error:'해당 장부가 없습니다.'},404);
  return json({ok:true});
