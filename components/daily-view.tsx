...
export default function DailyView(){
 ...
 const refresh=useCallback(async()=>{setLoading(true);setError('');try{const r=await fetch('/api/daily',{cache:'no-store'});if(r.status===401){window.location.assign('/login');return;}const d=await r.json();if(!r.ok)throw Error(d.error);setDates(d.reports);setRevision(v=>v+1);setDate(current=>current||d.reports[0]?.date||'');}catch(e){setError(e instanceof Error?e.message:'목록을 불러오지 못했습니다.');}finally{setLoading(false);}},[]);
 useEffect(()=>{void refresh();},[refresh]);
 useEffect(()=>{if(!date)return;const controller=new AbortController();setReport(null);setError('');fetch(`/api/daily?date=${date}`,{signal:controller.signal,cache:'no-store'}).then(async r=>{if(r.status===401){window.location.assign('/login');return;}const d=await r.json();if(!r.ok)throw Error(d.error);setReport(d.report);}).catch(e=>{if(e.name!=='AbortError')setError(e.message);});return()=>controller.abort();},[date,revision]);
 return <main className="daily-shell"><header className="daily-heading"><div><a href="/">← 현재 재고로</a><h1>일별 재고</h1><p>날짜별로 보관한 품목과 수량을 확인합니다.</p></div><form action="/api/logout" method="post"><button className="daily-button">로그아웃</button></form></header>
